...
rectangle_t build_rectangle(point_t p0, point_t p1)
{
    ...
    if (p0.x < p1.x) {
        ...
        rect.bottom_rigth.x = p1.x;
    } else {
        rect.top_left.x = p1.x;
        rect.bottom_rigth.x = p0.x;
    }

    if (p0.y < p1.y) {
        rect.top_left.y = p0.y;
        rect.bottom_rigth.y = p1.y;
    } else {
        rect.top_left.y = p1.y;
        rect.bottom_rigth.y = p0.y;
    }
...
int get_rectangle_area(rectangle_t rect)
{
    ...
    w = abs(rect.bottom_rigth.x - rect.top_left.x);
    h = abs(rect.bottom_rigth.y - rect.top_left.y);
